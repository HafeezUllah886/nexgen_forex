        ...
        <div id="sidebar-menu" class="sidebar-menu">
            <ul>
                ...
                <li {{ request()->routeIs('accounts.index') ? 'class=active' : '' }}>
                    <a href="{{ route('accounts.index') }}"><i class="ti ti-wallet fs-16 me-2"></i><span>{{ __('messages.accounts') }}</span></a>
                </li>
                <li {{ request()->routeIs('reports') ? 'class=active' : '' }}>
                    <a href="{{ route('reports') }}"><i class="ti ti-chart-bar fs-16 me-2"></i><span>{{ __('messages.reports') }}</span></a>
                </li>
                <li class="menu-title"><span>{{ __('messages.settings') }}</span></li>
                <li {{ request()->routeIs('profile') ? 'class=active' : '' }}>
                    <a href="{{ route('profile') }}"><i class="ti ti-user fs-16 me-2"></i><span>{{ __('messages.profile') }}</span></a>
                </li>
                <li {{ request()->routeIs('users.index') ? 'class=active' : '' }}>
                    <a href="{{ route('users.index') }}"><i class="ti ti-users fs-16 me-2"></i><span>{{ __('messages.users') }}</span></a>
                </li>
                <li>
                    <a href="javascript:void(0)" onclick="document.getElementById('logout-form').submit()"><i class="ti ti-logout fs-16 me-2"></i><span>{{ __('auth.logout') }}</span></a>
                </li>
            </ul>
        </div>
    </div>
